...
export class ErrorHandler {
     ...
     */
    static handleFileSystemError(error, operation, toolName) {
        ...
        switch (error.code) {
            case 'ENOENT':
                throw new McpError(ErrorCode.InvalidRequest, `Requested content not found for ${toolName}`);
            case 'EACCES':
                throw new McpError(ErrorCode.InternalError, `Access denied while processing ${toolName}`);
            case 'EMFILE':
            case 'ENFILE':
                throw new McpError(ErrorCode.InternalError, `System resource limit reached while processing ${toolName}`);
            case 'EISDIR':
                throw new McpError(ErrorCode.InvalidRequest, `Invalid content type requested for ${toolName}`);
            case 'ENOTDIR':
                throw new McpError(ErrorCode.InvalidRequest, `Invalid path structure for ${toolName}`);
            default:
                throw new McpError(ErrorCode.InternalError, `An unexpected error occurred while processing ${toolName}`);
        }
    }
    /**
     ...
     */
    static handleDirectoryListingError(error, dirPath) {
     ...
     */
    static validateFileSize(filePath, stats, maxSize = 1024 * 1024) {
        if (stats.size > maxSize) {
            throw new McpError(ErrorCode.InvalidRequest, `File too large. Maximum size allowed: ${Math.round(maxSize / 1024)}KB`);
        }
    }
    /**
     ...
     */
    static formatSafeErrorMessage(error, context) {
 ...
 */
export function createAuditLog(level, operation, details) {
    const logEntry = {
        timestamp: new Date().toISOString(),
        level,
        operation,
