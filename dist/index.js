    ...
    try {
        ...
        switch (toolName) {
            case "get_sveltekit_doc": {
                const docArgs = args;
                const content = await fileService.readSecureFile(CONFIG.svelteKitDocsPath, docArgs.topic, '.md');
                createAuditLog('info', 'sveltekit_doc_served', { topic: docArgs.topic });
                return { content: [{ type: "text", text: content }] };
            }
            case "get_tailwind_info": {
                const infoArgs = args;
                const content = await fileService.readSecureFile(CONFIG.tailwindDocsPath, infoArgs.query, '.md');
            ...
            case "get_component_snippet": {
                ...
                const content = await fileService.readSecureFile(categoryPath, snippetArgs.snippet_name, '.svelte');
        ...
        throw new McpError(ErrorCode.InternalError, ErrorHandler.formatSafeErrorMessage(error, toolName));
    }
});
/**
